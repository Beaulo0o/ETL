...
| ![MinIO](docs/images/weather.png) | JSON погоды в MinIO |
...
│ └── ci.yml # автотесты при push и PR
│
├── dags/ # Airflow DAGи
│ ├── weather_ingestion.py # сбор погоды с OpenWeatherMap API
│ ├── sales_transform.py # трансформация и загрузка продаж
│ └── reporting_daily.py # генерация Excel-отчёта
│
├── plugins/ # кастомные расширения Airflow
│ ├── hooks/
│ │ └── weather_api_hook.py # коннектор к Weather API
│ └── operators/
...
│ ├── transformations/ # ETL-трансформации
...
├── scripts/ # утилиты
│ └── data_generator/
│ └── generate_synthetic_sales.py # генератор фейковых чеков
│
├── tests/ # PyTest
│ ├── dags/
...
│ ├── docker-compose.yaml # все сервисы (Airflow, Postgres, MinIO, Redis)
│ ├── Dockerfile.airflow # кастомный образ Airflow
│ └── config/
│ └── init-multiple-dbs.sh # инициализация PostgreSQL
│
